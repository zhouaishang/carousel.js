...
var min=loop?step:0;var max=loop?slidercountinview+step:slidercountinview;carouselscroll.childNodes.forEach(function(node,i){if(node.nodeType===1){node.style.width=node.offsetWidth+'px';node.style.height=node.offsetHeight+'px';}
if(i>=min&&i<max)
node.classList.add(carouselscrollactiveclass);});var lastsliderrect=carouselscroll.lastElementChild.getBoundingClientRect();if(direction==='x'){scrollwidth=lastsliderrect.right-carouselleft+positionoffset;scrollheight=carouselheight;}else{scrollwidth=carouselwidth;scrollheight=lastsliderrect.bottom-carouseltop+positionoffset;}
...
carouselscroll.childNodes.forEach(function(c,i){if(i>=index&&i<index+slidercountinview)
c.classList.add(carouselscrollactiveclass);else
c.classList.remove(carouselscrollactiveclass);});in_transition=false;},duration);}

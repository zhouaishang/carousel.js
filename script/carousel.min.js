...
var carousel=new Carousel(carouselscroll,duration,step,loop,direction,indicator,activeclass,nextbutton,previousbutton,mousewheel);if(autoplay){carousel.autoplay(delay);}});}
function Carousel(carouselscroll,duration,step,loop,direction,indicator,activeclass,nextbutton,previousbutton,mousewheel){var _this=this;var in_transition=false;var eligible=true;var autoplayid;var carouselleft;var carouselright;var carouseltop;var carouselbottom;var carouselwidth;var carouselheight;var scrollY;var scrollX;var scrollwidth;var scrollheight;var slidercount;var slidercountinview;var step=step;var currentindex=0;function get_offset(){if(arguments.length==1){var index=arguments[0];direction=direction;}else if(arguments.length==2){var index=arguments[0];direction=arguments[1];}else{return false;}
var carousel=carouselscroll.children;if(index>carousel.length)return false;var slider=carousel[index];var rect=slider.getBoundingClientRect();if(direction=='x')
return rect.left-carouselleft-(scrollX-w.scrollX);else
return rect.top-carouseltop-(scrollY-w.scrollY);}
function get_slider_count_in_view(){var sliders=carouselscroll.children;var slidercount=sliders.length;var containernum=(direction==='x')?carouselwidth:carouselheight;for(var i=0;i<slidercount;i++){var num=get_offset(i);if(num==containernum){return i;}
else if(num>containernum){return i-1;}}}
...
carouselscroll.childNodes.forEach(function(node,i){if(node.nodeType===1){node.style.width=node.offsetWidth+'px';node.style.height=node.offsetHeight+'px';}});var lastsliderrect=carouselscroll.lastElementChild.getBoundingClientRect();if(direction==='x'){scrollwidth=lastsliderrect.right-carouselleft+positionoffset;scrollheight=carouselheight;}else{scrollwidth=carouselwidth;scrollheight=lastsliderrect.bottom-carouseltop+positionoffset;}
...
index=index+step;else if(index>slidercount+step)
index=index-slidercount;else
...
`;w.document.head.append(nestedstyle);})(window);
